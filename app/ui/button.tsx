...
const buttonVariants = cva(
  "inline-flex items-center gap-2 justify-center whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50",
  {
    variants: {
      variant: {
        default:
          "bg-cyan-600 text-white shadow hover:bg-cyan-600/90",
        outline:
          "border border-gray-200 bg-white shadow-sm hover:bg-gray-200",
        secondary:
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, children, type, size, disabled, isLoading: clientLoading = false, asChild = false, ...props }, ref) => {
