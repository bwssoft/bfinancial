...
export async function POST(request: Request) {
  ...
  if (!payment)
    return new Response("No payment with this code", {
      status: 500,
      statusText: "No payment with this code",
    });
  if (!payment?.is_detached) {
    ...
    const offer = await getCachedOffer(
      omie_metadata.enterprise,
      parseInt(omie_metadata.codigo_pedido!)
    );
    if (!offer)
      return new Response("No offer with this code", {
        status: 500,
        statusText: "No offer with this code",
      });
    ...
    if (offer.pedido_venda_produto.cabecalho.etapa !== "60") {
      ...
      const codigo_cenario_impostos =
        offer.pedido_venda_produto.cabecalho.codigo_cenario_impostos;
      const categoria_adiantamento =
        categoriaAdiantamentoCod[OmieEnterpriseEnum[omie_metadata.enterprise]];
      const codigo_imposto =
        cCodImposto[codigo_cenario_impostos as keyof typeof cCodImposto];
      const parcela = _offer.lista_parcelas.parcela.map((parcela) => {
        if (parcela.numero_parcela === omie_metadata.numero_parcela) {
          return {
            ...
            categoria_adiantamento:
              categoria_adiantamento[
                codigo_imposto as keyof typeof categoria_adiantamento
              ],

            conta_corrente_adiantamento:
              nCodCCByEnterprise[OmieEnterpriseEnum[omie_metadata.enterprise]],
          };
        }
        return parcela;
      });
    ...
    if (!receive_orders)
      return new Response("No receive offer with this code", {
        status: 500,
     ...
     */
    const [current_receive_order] =
      receive_orders.conta_receber_cadastro.filter(
        (a) =>
          Number(a.nCodPedido) === Number(omie_metadata.codigo_pedido) &&
          Number(a.numero_parcela.split("/")[0]) ===
            Number(omie_metadata.numero_parcela)
      );

    if (!current_receive_order)
      return new Response("No receive offer linked", {
        status: 500,
...
const nCodCCByEnterprise = {
  ...
  PRMA: "11097908573", // <-- Não encontrei na OMIE
};
...
//https://app.omie.com.br/developer/api-test/?ENDPOINT=https%3A%2F%2Fapp.omie.com.br%2Fapi%2Fv1%2Fgeral%2Fcenarios%2F&OMIE_CALL=ListarCenarios&PARAMS=%257B%2B%2B%2522nPagina%2522%253A%2B1%252C%2B%2B%2522nRegPorPagina%2522%253A%2B20%257D
const cCodImposto = {
  ...
  "3506002691": "PADRAO", // HYBRID Padrão
  "3506002920": "VENDA", // HYBRID 5102-6102/VENDA
